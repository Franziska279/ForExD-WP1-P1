...
def extract_median_percentiles_slope_distance_per_VI(data_ids, data_refdm, result_path):
    ...
    print(f"Processing USDA_IDX: {usda_idx}")

    # Check if 'mini_idx' is None or NaN
    if pd.isna(row['mini_idx']):
        print(f"No mini_idx found for USDA_IDX: {usda_idx}. Skipping...")
...
def main(ids_path, refdm_path, disturbance_type, dir_path, output_path, slurm_array_task_id):
    logger = logging.getLogger(__name__)
    
    logger.info("Define Paths...")
    result_path = f'/{output_path}/{slurm_array_task_id}_slope_distance.csv'
    ...
    refdm_idx = refdm.loc[slurm_array_task_id]

    logger.info(f"Extract median, percentiles, slope and distance for each vegetation index ...")
    extract_median_percentiles_slope_distance_per_VI(data_ids=ids, 
                                                     data_refdm=refdm_idx, 
                                                     result_path=result_path) 
    
    # logger.info(f"Load result ...")
    # results_df = gpd.read_file(result_path)
...
if __name__ == "__main__":
    ...
    disturbance_type = sys.argv[3]
    dir_path = sys.argv[4]
    output_path = sys.argv[5]
    slurm_array_task_id = sys.argv[6]
    index = int(slurm_array_task_id)

    main(ids_path, refdm_path, disturbance_type, dir_path, output_path, index)